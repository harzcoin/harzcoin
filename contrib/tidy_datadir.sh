#!/bin/bash
# Copyright (c) 2013 The Harzcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Harzcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Harzcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Harzcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Harzcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Harzcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Harzcoin 0.8 datadir."
    ;;
esac
