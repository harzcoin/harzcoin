...
The RPM spec file provided here is for Harzcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://harzcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Harzcoin git repository](https://gitlab.com/harzcoin/harzcoin) but are not part of
...
Currently only `Source10` is used, for the example `harzcoin.conf` file.
...
in the harzcoin git repository. At present only an SVG version of the Harzcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the harzcoin
...
needed when 0.12.1 is released, a proper fix is already in the Harzcoin git
master branch.
...
idea to build Harzcoin Core against a static version of that library compiled
...
The `harzcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `harzcoin-server` package. At this point, most RPM based
...
The `harzcoin-server` package should have SELinux support. How to properly do
...
Harzcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/harzcoin/). He has been
packaging Harzcoin for Fedora at least since 2012.
...
4. I renamed the `harzcoin` package that contains the Qt GUI to `harzcoin-core` as
that appears to be how the general population refers to it, in contrast to
`harzcoin-xt` or `harzcoin-classic`. I wanted to make sure the general population
