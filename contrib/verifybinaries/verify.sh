...
###   core.harzcoin.org and harzcoin.org and compares them.
...
WORKINGDIR="/tmp/harzcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://core.harzcoin.org"
HOST2="https://harzcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="harzcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'harzcoin-',
...
if [ $? -ne 0 ]; then
   echo "harzcoin.org failed to provide signature file, but core.harzcoin.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "harzcoin.org and core.harzcoin.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Harzcoin Core binary release signing key installed?"
   fi
